...
| 模型与成本 | 基础良好 | 模型档位、根调用/turn/failover/repair/tool-write 预算、预授权与按 token 结算均已具备；租户级预算与容量策略仍缺失 |
...
当前治理已经覆盖用户级限流、用户级非终态任务上限、模型并发、任务预算、积分预授权和按 token 结算。对于个人产品足够，但团队/企业租户仍存在两个运营风险：
...
在现有 Admission Policy 前增加组合策略，不改生成内核：

1. `TenantGenerationQuotaPolicy` 同时检查租户运行中任务、排队任务、周期积分和临时资源水位。
2. 额度按 `tenantId + billingPeriod + policyVersion` 原子预留，任务终态复用现有结算与退款机制。
3. 超限响应区分：拒绝、排队、降级模型档位、禁止 Heavy；高风险任务不得为了省钱降级验证等级。
4. 管理端暴露预算使用、单位成功任务成本、失败浪费成本和阈值告警。
5. 调度按租户公平，不以简单全局 FIFO 让大租户长期占满 worker。
...
- 并发压测证明单租户不能突破配额，也不能饿死其他租户。
- 预算预留、幂等重试、取消退款和恢复结算在并发下账实一致。
- 告警指标：租户排队 P90、预算使用率、拒绝/降级数、单位成功交付成本。
...
| 8 | 租户并发和周期预算准入策略 | 压测下租户隔离、账实一致、公平排队 |
