...
- 2026-08-28 提交 `48f5fba`：新增 `GET /generation/tenants/{tenantId}/control-plane` 版本 1 合同，只返回租户月预算、剩余额度、低敏排队汇总、按实际 route 与目标代码类型聚合的单位成功成本，以及当前生效的准入阻断项。
- 控制面服务在任何租户聚合查询前调用持久租户成员关系校验并要求 `TenantRole.ADMIN`；`OWNER` 由既有角色包含关系放行，普通成员直接拒绝且不会触发租户级成本查询。授权与聚合运行在同一只读事务边界内。
- 月预算复用准入链唯一的不可变积分流水投影，不复制第二份预算 SQL；排队口径与准入一致，只统计 `queued/running/waiting_approval`，Heavy 容量只统计实际 `heavy_generation` 路由。
- 单位成功成本沿用既有经济性定义：分子包含同场景全部已结算尝试的积分成本，分母只使用成功交付数；同时返回已结算任务数、成功数和总积分成本，失败率上升时不会把单位成功成本错误稀释。
...
### 11.10 应用级生成控制与预算门禁证据

- 2026-08-28 提交 `2039b3c`：新增版本化 `app_generation_control` 事实源和 `GET/PUT /generation/apps/{appId}/control`；读取与完整替换均先校验应用存在及租户管理员权限，写入使用应用行锁、`expectedVersion` 乐观锁和最后更新人/时间，不在 Controller、工具或执行器中复制所有权规则。
- 控制快照覆盖暂停新任务、紧急停止、应用最大并发、`PLATFORM_DEFAULT/ECONOMY_ONLY` 模型策略、依赖清单修改、依赖网络、危险工具和应用月积分上限。当前应用仍共享单个可写工作区，因此数据库和领域模型共同把最大并发限制为 `1`；只有任务级工作区隔离完成后才能扩大，不用配置掩盖并发覆盖风险。
- 准入在模型成本发生前按固定“租户→用户→应用”顺序锁定真实行，并在同一事务读取应用控制与不可变积分流水、执行预算判断和写入预授权。预检流水从首次冻结起携带 `appId`，结算、恢复和幂等重放逐项校验 `userId + tenantId + appId + taskId`，两个独立连接池并发冻结 `3 + 3`、应用上限 `5` 时恰好一个成功，最终余额 `97`、应用用量 `3`、孤儿流水为 `0`。
- 普通暂停只拒绝新提交，不伪造运行中任务已停止；紧急 kill switch 在策略落库后以 `app_emergency_stop` 原因取消当前唯一活动任务，并在执行规划、每次模型回合、全部工具调用和依赖安装重试前再次失败关闭。危险工具策略只能从持久审批收紧为禁止，不能绕过已有 request epoch、参数摘要和一次性审批合同。
- `ECONOMY_ONLY` 只允许把质量档降为平衡档且关闭 thinking，不能反向提升平台选择；依赖修改和依赖网络策略只会收紧已有边界。生成代码运行期仍由平台 `RUNTIME_INTERNAL` 网络策略禁止外部访问，本轮没有新增应用级外网放行能力；应用只能进一步禁止受信 registry 的依赖 egress。
...
- 2026-08-28 提交 `c7160b6`：新增[生成控制面威胁模型](./生成控制面威胁模型.md)，按主体、HTTP/内部恢复入口、鉴权与业务服务、审计服务、MySQL、日志/指标划分信任边界，逐项记录越权、审计绕过、敏感信息泄漏、结果伪造、选择性删除和存储故障的控制、残余风险与处置责任；该文档是当前代码事实的威胁建模，不宣称完成渗透测试。
- `GenerationControlAuditAspect` 位于生成控制 RBAC 外层，受控调用先写 `STARTED` 再进入鉴权和业务；成功、拒绝、业务驳回与异常分别收敛为 `SUCCESS/DENIED/REJECTED/FAILED`，进程中断或结果写入失败保留 `STARTED` 表示结果未知。开始记录失败时失败关闭，已成功业务不会因结果补写失败被重复执行。
- 九类权限矩阵操作均声明受信资源类型和有界资源表达式，现代/旧版提交与应用删除记录 `APP`，任务查询、取消、危险工具审批、SSE/终态重放记录 `TASK`，模型配置与 Benchmark 记录平台资源；内部自动恢复通过系统主体记录 `TASK_RECOVERY`，没有新增可绕过授权的恢复 Controller。
- 审计事实仅保存事件 ID、权限、资源类型/ID、主体类型/用户 ID、传输、结果码和时间，不保存请求体、提示词、异常文本、IP、User-Agent、session 或凭据；资源表达式由架构测试限制为参数、受限属性或有界字面量，失败日志只含事件 ID 与异常类型。
- 每条事件在创建时冻结 90 天到期时间，后台每小时最多删除 500 条到期记录；应用接口只暴露开始、一次性完成和到期批量删除，不提供按主体或资源选择性删除的 HTTP 能力。终态重放、AI 发布等已有原子审计继续承担各自的幂等/CAS 事实，通用控制审计不替代业务原子性。
