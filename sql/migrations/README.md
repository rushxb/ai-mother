...
`V20260828_5__generation_control_audit.sql` 新增生成控制面脱敏审计事件。
受控操作必须先在独立事务写入 `STARTED`，再单向终结为有界结果；进程中断或结果回写失败时保留 `STARTED` 表示结果未知，禁止人工改写为成功或失败。
事件只保存权限、主体编号、app/task/平台资源编号、交通边界、有界结果码和时间；不得写入请求体、提示词、异常消息、凭据、IP、User-Agent 或 Session。
过期数据只允许按 `expiresAt` 使用有界批次删除；删除前禁止业务级选择性修改或软删除。
