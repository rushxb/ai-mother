...
public class AiModelRuntimeProperties {
    ...
    private Duration firstSignalTimeout = Duration.ofSeconds(45);
    ...
    /** 启动影子候选前等待主候选输出的时间。 */
    private Duration firstTokenHedgeDelay = Duration.ofSeconds(3);

    /** 是否要求主候选和影子候选来自不同供应商。 */
    private boolean firstTokenHedgeRequireDistinctProvider = true;

    /** 根模型尝试失败后，刷新健康模型池前的最小等待时间。 */
    private Duration rootModelRetryMinDelay = Duration.ofSeconds(3);

    /** 根模型重试指数退避允许达到的最大等待时间。 */
    private Duration rootModelRetryMaxDelay = Duration.ofSeconds(20);

    /** 根模型重试退避的随机抖动比例。 */
    @DecimalMin("0.0")
    @DecimalMax("1.0")
    private double rootModelRetryJitter = 0.35;
