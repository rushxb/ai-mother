 ...
 */
    public CreateGenerationPlan plan(CodeGenTypeEnum codeGenType, String userMessage) {
        ...
        return switch (codeGenType) {
            case VUE_PROJECT -> planVue(userMessage);
            case BACKEND_PROJECT -> planBackend(userMessage);
            case FULL_STACK_PROJECT -> planFullStack(userMessage);
            default -> unsupported(codeGenType, "CREATE 模板运行时暂不覆盖该代码类型");
        };
    }
    ...
    private CreateGenerationPlan planVue(String userMessage) {
        ...
        if ("vue-web-landing".equals(baseTemplate)) {
            return build(CodeGenTypeEnum.VUE_PROJECT, baseTemplate,
    ...
    private CreateGenerationPlan planFullStack(String userMessage) {
        ...
        List<FeatureModuleManifest> modules = new ArrayList<>();
        modules.add(defaultFrontendModule(frontendTemplate));
        modules.add(frontendCrudAdminModule(frontendTemplate));
        if (containsAny(userMessage, "登录", "注册", "auth", "权限", "用户")) {
            modules.add(authModule(frontendTemplate));
        }
        if (containsAny(userMessage, "商品", "产品", "product", "库存")) {
            modules.add(commerceModule(frontendTemplate));
        }
        if (containsAny(userMessage, "预约", "排班", "日程", "课程", "门店", "到店", "booking", "schedule")) {
            modules.add(bookingModule(frontendTemplate));
        }
        modules.add(module("backend-crud-api", "Go SQLite CRUD API", BACKEND_TEMPLATE,
                List.of("domain_contract", "module_model", "module_repository", "module_service", "module_handler",
                        "database_schema", "module_import", "server_wiring"),
                "全栈 CRUD 需要后端 API、schema 和启动装配"));
        return build(CodeGenTypeEnum.FULL_STACK_PROJECT,
                frontendTemplate + "+" + BACKEND_TEMPLATE,
                "Vue + Go SQLite 全栈 CRUD 模板计划",
                modules);
    }
    ...
    private CreateGenerationPlan build(CodeGenTypeEnum codeGenType,
        ...
        for (FeatureModuleManifest module : modules) {
            ...
            groups.add(new SlotGroup(module.moduleId() + "-slots", module.templateId(), module.moduleId(), slotIds, order++));
        }
        return new CreateGenerationPlan(
                codeGenType,
                new CreateTemplateManifest(baseTemplateId, codeGenType, reason),
                modules,
                groups,
                0.82,
                reason,
    ...
    private FeatureModuleManifest module(String moduleId, String name, String templateId, List<String> slotIds, String reason) {
    ...
    private CreateGenerationPlan unsupported(CodeGenTypeEnum codeGenType, String reason) {
        return new CreateGenerationPlan(codeGenType, null, List.of(), List.of(), 0, reason,
